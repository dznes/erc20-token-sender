...
fn test_burn_token() {
    ...
    assert(balance_before == INITIAL_SUPPLY, 'Invalid balance');

    let burn_value: u256 = 100;
    erc20.burn(account, burn_value);

    let balance_after = erc20.balance_of(account);
    assert(balance_after == INITIAL_SUPPLY - burn_value, 'Invalid balance');
}
...
#[test]
fn test_get_generation_timestamp() {
    let contract_address = setup();
    let erc20 = IERC20Dispatcher { contract_address };

    let generation_timestamp = erc20.get_generation_timestamp();
    let felt_timestamp: felt252 = generation_timestamp.try_into().unwrap();

    println!("Block creation timestamp: {}", felt_timestamp);
    assert(generation_timestamp == 0, 'Should be 0')
}
...
#[test]
#[fork(
    url: "https://starknet-sepolia.public.blastapi.io/rpc/v0_7", block_id: BlockId::Number(74816)
)]
